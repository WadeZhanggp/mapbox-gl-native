...
#include <mbgl/renderer/circle_render_data.hpp>
...
CircleBucket::CircleBucket(MapMode mode_)
    : mode(mode_), renderData(std::make_unique<CircleRenderData>()) {
}
...
void CircleBucket::upload(gl::ObjectStore& store) {
    if (renderData) {
        ...
        renderData->elementsBuffer.upload(store);
    }
    uploaded = true;
}
...
void CircleBucket::addGeometry(const GeometryCollection& geometryCollection) {
    ...
    auto& elementsBuffer = renderData->elementsBuffer;
    ...
    for (auto& circle : geometryCollection) {
        for(auto & geometry : circle) {
            ...
            if (!triangleGroups.size() || (triangleGroups.back()->vertex_length + 4 > 65535)) {
                ...
                triangleGroups.emplace_back(std::make_unique<CircleRenderData::TriangleGroup>());
            }
