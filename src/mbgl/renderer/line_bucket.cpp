...
#include <mbgl/renderer/line_render_data.hpp>
...
LineBucket::LineBucket(uint32_t overscaling_)
    : overscaling(overscaling_), renderData(std::make_unique<LineRenderData>()) {
}
...
void LineBucket::addGeometry(const GeometryCoordinates& vertices) {
    ...
    // Store the triangle/line groups.
    {
        if (triangleGroups.empty() || (triangleGroups.back()->vertex_length + vertexCount > 65535)) {
            ...
            triangleGroups.emplace_back(std::make_unique<LineRenderData::TriangleGroup>());
        }
...
bool LineBucket::hasData() const {
    return renderData && !renderData->triangleGroups.empty();
}
